...
namespace Sql_Labb3
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("8. Give Grades");

            Console.WriteLine("9. Courses");

            Console.WriteLine("10. Average Budget");

            Console.WriteLine("11. Department Payout");

            Console.WriteLine("12. Amount of teachers");
            ...
            switch (input)
            {
                ...
                case 8:
                    ...
                    int coursenr = Int32.Parse(Console.ReadLine());

                    Console.WriteLine("Student number");

                    int studentnr = Int32.Parse(Console.ReadLine());

                    Console.WriteLine("Grading");

                    int grade = int.Parse(Console.ReadLine());

                    Console.WriteLine("By which teacher");
                    ...
                    StudentGrade studentGrade = new StudentGrade { CourseId = coursenr, StudentId = studentnr, Grade = grade, Teacher = teacher, Published = published };
                ...
                case 9:
                    ...
                    var onSiteCourses = context.OnsiteCourses
                                            .Where(p => p.CourseId != null)
                                            .OrderBy(p => p.CourseId);
                    foreach (OnsiteCourse p in onSiteCourses)
                    {
                        Console.WriteLine($"Course ID:     {p.CourseId}");
                        Console.WriteLine($"Location:     {p.Location}");
                        Console.WriteLine($"Days:     {p.Days}");
                ...
                case 10:
                    ...
                    var avg1 = context.Departments
                              .Where(p => p.DepartmentId > 0)
                            .Average(p => p.Budget);

                    Console.WriteLine($"Average Budget:     {avg1}");

                    break;
                case 11:

                    var payouts = context.Departments
                                            .Where(p => p.DepartmentId != null)
                                            .OrderBy(p => p.DepartmentId);
                    foreach (Department p in payouts)
                    {
                        Console.WriteLine($"Department ID:     {p.DepartmentId}");
                        Console.WriteLine($"Name:     {p.Name}");
                        Console.WriteLine($"Budget:     {p.Budget}");
                        Console.WriteLine($"Start date:     {p.StartDate}");
                        Console.WriteLine($"Administrator:     {p.Administrator}");
                    }
                    break;
                case 12:

                    var teachers = context.People
                                            .Where(p => p.Discriminator == "Teacher").Count();

                    Console.WriteLine("Teacher Count: " + teachers);
